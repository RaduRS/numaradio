...
## Reconciler race: re-pushes staged QueueItems that already played

**Status:** parked 2026-05-16.
**Resume trigger:** before the next time we change anything in the
`priority_request` insert path, OR if a stale-item-replay incident
repeats.

### Symptom

`workers/queue-daemon/reconciler.ts` re-pushes a `priority_request`
`QueueItem` row to Liquidsoap even after the track has played, when:
- DB row's `queueStatus` is still `staged` (transition to `playing`/
  `completed` lagged or never fired)
- Liquidsoap's in-memory `priority.queue` doesn't contain the URL
  (because the track was already popped to play)
- Reconciler reads "0 items in Liquidsoap, 2 staged in DB" → re-pushes

Real prod log line (2026-05-16 20:50:06):
```
[reconciler] tick: staged=2 liquidsoap=0 repushed=2
```

Caused 1 incident tonight where a listener-requested track risked
re-airing. Caught early by manually cancelling the stale row.

### Root cause hypothesis

The reconciler's idempotency check (`if (liquidsoapUrls.has(url)) continue`)
is correct for the "telnet dropped on the way in" case it was designed
for. But it doesn't distinguish between:
- (A) URL wasn't pushed yet (legitimate retry case)
- (B) URL was pushed AND already played (Liquidsoap popped it)

For (B), the DB row should have transitioned `staged → playing → completed`
via the daemon's `track-started` callback handler. If that transition
fails or lags, reconciler treats it as case (A) and re-pushes.

### Fix options

1. **Check NowPlaying + recent PlayHistory before re-push.** If the
   item's trackId is currently playing OR was played in the last N
   minutes, skip the re-push (it's case B). Cheap query, fully idempotent.
2. **Investigate the `track-started` callback's status transition.**
   Why didn't `cmp8rf5o` transition from `staged` to `playing` after
   the track aired? Race with the dashboard's `/api/internal/track-started`
   route? Missing field on Phase 5b-inserted rows?
3. **Add a max-age cutoff on reconciler re-push.** If an item is older
   than, say, 30 min and still `staged`, assume it played + status
   never transitioned, and mark it `completed` instead of re-pushing.

Option 1 is the safest immediate fix. Option 2 is the proper root-cause
dive. Option 3 is a stop-gap that protects against any future variant
of the same class of bug.

Recommend: ship Option 1 immediately, then dig into Option 2.

### Files likely touched

- `workers/queue-daemon/reconciler.ts` — add the NowPlaying / recent-
  PlayHistory check before re-pushing
- `app/api/internal/track-started/route.ts` OR `workers/queue-daemon/index.ts`
  `onTrackHandler` — investigate the status-transition path

### Estimated size

Option 1: ~3-5 tasks (small).
Option 2 + 3 together: ~6-8 tasks.

### Cross-cuts

- Same root cause class as the Fault Lines loop incident (already fixed
  the worst trigger — Phase 5b now goes through pushHandler — but
  reconciler is still a latent loop vector).
- LENA_REQUEST_AUTONOMY and LENA_QUEUE_AUTONOMY can both stay ON in
  the interim — the manual stale-cancel script kept things safe tonight
  and we now know what to look for in logs (`reconciler tick: staged=N
  liquidsoap=0 repushed=N` where N>0).
- Add a daemon-side log alert if `repushed > 0` for the same trackId
  twice in a row → that's a clear smoke signal.
