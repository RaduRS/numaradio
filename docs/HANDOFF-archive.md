...
Sections rotated out of `docs/HANDOFF.md` once they stopped being
actionable. Kept verbatim so the operational detail (why a thing was
built, which env var bit us, which pipeline quirk only reveals itself
at launch) stays findable via grep. When picking up work, read
`docs/HANDOFF.md` first; fall back here when the main file references
something deeper.
...
## Foundations + earlier LIVE state (through 2026-04-21)
...
- "Play Next" button per row → `POST /api/library/push` → forwards to
  `http://127.0.0.1:4000/push`. Reason recorded as `dashboard:<cf-access-email>`.
- "Recent priority pushes" panel reads the daemon's `/status` endpoint
  (`lastPushes` + `lastFailures`), polled every 5s.
- 18 unit tests for `dashboard/lib/library.ts`.
...
- ✅ `liquidsoap/numa.liq` uses `fallback(track_sensitive=true, [priority, rotation, blank()])`.
...
- ✅ Dashboard bind fix: `dashboard/package.json` start script is
...
- **Note:** voice model was swapped Asteria → Andromeda → Luna in later
  iterations; see the Lena voice-tuning section of the main HANDOFF.
...
- ✅ `dashboard.numaradio.com/shoutouts` — three cards:
  - **Compose**: textbox + "Send to Lena" (⌘/Ctrl+Enter). POSTs to
    ...
    Sender tagged `dashboard:<cf-access-email>` in Track provenance.
...
SessionUnlock), runs as S4U, and its action **keeps `wsl.exe`
...
persistently attached instead of relying on the idle-timeout knob.
...
First unattended-restart test exposed two bugs:
- Every previously-aired Lena shoutout re-played back-to-back when the queue
  ...
  straight away (fire-and-forget to an in-memory overlay queue; no `staged`
  phase to resume), and `hydrate()` explicitly skips `queueType='shoutout'`
...
- The public `<audio>` element gave up on the first `error` event and showed
  ...
  now tracks a `wantPlaybackRef` (user pressed Play) and on `error` stays
...
- ✅ **Phase 1** — `NowSpeaking` migration applied to Neon; broadcast /
  ...
  stub at the time — has since shipped, see Song Generation section above).
...
- ✅ **Phase 3** — `liquidsoap/numa.liq` is now:
