...
//
// Thin proxy to the public site's internal withdraw endpoint.
// Operator clicks "Withdraw" on an approved row; the public side
// pulls the track from rotation, deletes audio assets, and either
// keeps or scrubs the email/name based on the submitter's original
// airing preference (permanent → keep, one_off → scrub). Reason is
// optional.
...
export async function POST(
  ...
  let res: Response;
  try {
    res = await fetch(`${PUBLIC_SITE}/api/internal/submissions/${id}/withdraw`, {
      method: "POST",
      headers: {
